...
class Ad extends Common
{
    ...
    public function index(){
        if(Request::isAjax()) {
            ...
            $list = Db::table('ad')->alias('a')
                ->join('adsense at', 'a.as_id = at.as_id', 'left')
    ...
    public function add(){
        if(Request::isAjax()) {
            ...
            $data['addtime'] = time();
            if($data['endtime'] == ''){
                $data['endtime'] = date("Y-m-d",strtotime("+1month",strtotime(date('Y-m-d',time()))));  
            }
            db('ad')->insert($data);
            $result['code'] = 1;
            $result['msg'] = '广告添加成功!';
    ...
    public function addType(){
        if(Request::isAjax()) {
            ...
            $this->assign('title',lang('add').lang('ad').'类别');
            $this->assign('info','null');
