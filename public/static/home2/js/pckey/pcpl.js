$(function(){
    ...
    select_change('#pctype');
    var shuju = [];
    trhoverstyle(4);
    $(".keytj").click(function(){
       $.post("/home/pckeywords/toplimit", {type:'keyword_plquerynum'}, function(res){
        if(res.code == 0){
            $.sendError(res.msg,1000);
            return false;
        }else if (res.code == 2){
          selectError('今日PC端排名批量查询次数已达上限，是否升级会员组获取更多次数');
        } else {
          var pcurl = $.trim($("#pcurl").val());
          var pctype = $("#pctype").val();
          var pckeywords = $("#pckeywords").val();
          var arrkw = pckeywords.split(/[\n,]/g)
          if(pcurl == ''){
          ...
          if(pckeywords ==''){
            ...
            if(pckeywords.indexOf('\n') >= -1){
              ...
              layui.use('layer', function(){
                ...
                $('.js').css('display','none');
                $.post("/pckeypla", {target:pcurl,keyword:pckeywords,engine_type:pctype,rank_plnum:rank_plsubmit}, function(data){
                  if (data.code == 0) {
                    $.sendWarning(data.msg, 1000);
                    return false;
                  }else{
                    $('#pckeywords').val(data.keyword2)
                    ...
                    for (var i in arrkw) {
                      ...
                      if (arrkw[i]) {
                        ...
                        shuju.push({
                          ...
                          '终端':'PC端',
                          '关键词':arrkw[i],
                          'PC端搜索量':data.sousl[i]?data.sousl[i]:'暂未查到搜索量',
                          '排名':'100+',
                          '预估流量':0
                        })
                      }  
                    ...
                    var timer = setInterval(function(){
                      if ((Date.parse(new Date())/1000 - starttime)>30) {
                      ...
                      $.post('/pckeyplb', {}, function(data){
                        if (data.code == 1) {
                          ...
                          for (var i in info) {
                              ...
                              if (pctype == 0) {
                                $('#rank'+res['keyword']).removeClass('norank').html('<a target="_blank" style="color:#423da1;" href="https://www.baidu.com/s?wd='+res['keyword']+'&rn=50">'+res['rank']+'</a>')
                              } else {
                                $('#rank'+res['keyword']).removeClass('norank').html(res['rank'])
                              }
                              
                              $('#flow'+res['keyword']).removeClass('noflow').html(flow)
    ...
    var numorder=1;
    var pmorder = $('.pldata').attr('data-pmorder');
        ...
        if (userlevel == '1') {
            ...
            onConfirm: function() {
              $.post('/getPoint', {}, function(point){
                if(point<2) {
                  ...
                  $('#daochuForm').submit()
                  // location.href = "/export2excel?shuju="+JSON.stringify(shuju)+"&title=电脑端批量排名导出&point=2"
                }
              }, 'json')
            },
            onCancel: function() {
          ...
          $.post('/exporttype', {exporttype:'rank_exportnum'}, function (data) {
              if (data.code == 1) {
                ...
                $('#daochuForm input[name=shuju]').val(JSON.stringify(shuju))
                $('#daochuForm').submit()
                // location.href = "/export2excel?shuju="+JSON.stringify(shuju)+"&title=电脑端批量排名导出&point=0"
              } else {
                $.sendConfirm({
                  hideHeader: true,
                  withCenter: true,
